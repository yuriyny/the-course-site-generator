...
public abstract class AppTemplate extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        alert.setTitle("Language/Мова");
        alert.setHeaderText("Choose Your Language");
        ButtonType buttonTypeOne = new ButtonType("English");
        ButtonType buttonTypeTwo = new ButtonType("Українська");
        alert.getButtonTypes().setAll(buttonTypeOne, buttonTypeTwo);
        Optional<ButtonType> result = alert.showAndWait();

        if ((result.isPresent()) && (result.get() == buttonTypeOne)) {

            APP_PROPERTIES_FILE_NAME = "app_properties.xml";
        } else {
            APP_PROPERTIES_FILE_NAME = "app_properties1.xml";
        }
        try {
